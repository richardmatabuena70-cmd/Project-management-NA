    ...
    document.getElementById(
        ...
        try {
            ...
            if (!formToken || formToken !== sessionStorage.getItem("hrms_form_token")) {
                ...
                toast("Invalid form submission. Please try again.");
            ...
            if (resumeFile && (resumeFile.size > 0 || resumeFile.name)) {
                ...
                try {
                    ...
                    toast("Failed to read resume file. Submitting without it.");

                }

            }

            let validIdDataURL = null;

            const validIdFile = f.get("validId");

            if (validIdFile && (validIdFile.size > 0 || validIdFile.name)) {

                try {

                    validIdDataURL = await readFileAsDataURL(validIdFile);

                } catch (err) {

                    console.error("ID upload error:", err);

                    toast("Failed to read ID file. Submitting without it.");
                        ...
                        <tr>
                            ...
                            <td class="actions">
                                ...
                                <button
                                    ...
                                    onclick="viewApplicant('${a.applicantId}')">
...
function myApplication() {
    ...
    return `
        ...
        <div class="panel">
            ...
            ${(a.resume || a.validId) ? `<div style="margin-top:12px; padding-top:12px; border-top:1px solid var(--border);"><b>My Documents</b><br><br>${[a.resume ? `<p><b>Resume:</b> <a href="${a.resume}" target="_blank" rel="noopener">📄 View Resume</a></p>` : "", a.validId ? `<p><b>Valid ID:</b> <a href="${a.validId}" target="_blank" rel="noopener">🪪 View ID</a></p>` : ""].filter(Boolean).join("")}</div>` : ""}
